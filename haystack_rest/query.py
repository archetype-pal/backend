...
class SpatialQueryBuilder(BaseQueryBuilder):
    ...
    def __init__(self, backend, view):
        ...
        assert getattr(self.backend, "point_field", None) is not None, (
            f"{self.backend.__class__.__name__}.point_field cannot be None. Set the "
            f"{self.backend.__class__.__name__}.point_field to the name of the `LocationField` "
            "you want to filter on your index class."
        )
        try:
