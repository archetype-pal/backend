...
class TestGraphViewSet(APITestCase):
    ...
    def test_bulk_hydration_fields_are_serialized(self):
        """`item_part`, `image_iiif` and `allograph_name` are what the bulk-edit
        client groups and previews by, so pin them to the row's own image."""
        response = self.client.get("/api/v1/manuscripts/graphs/")
        assert response.status_code == rest_framework.status.HTTP_200_OK, response.data

        row = next(item for item in response.data if item["id"] == self.graphs[0].id)
        assert row["item_part"] == self.item_image.item_part_id
        assert row["image_iiif"] == self.item_image.image.iiif.info
        assert row["allograph_name"] == self.allograph.name
    ...
    def test_viewer_write_response_carries_hydration_fields(self):
        """The bulk editor saves through this endpoint and re-renders from the
        response, so it has to echo the same grouping/label fields as the read
        endpoint."""
        response = self.client.post(
            "/api/v1/annotations/graphs/",
            data={
        ...
        assert response.status_code == rest_framework.status.HTTP_201_CREATED, response.data
        assert response.data["item_part"] == self.item_image.item_part_id
        assert response.data["allograph_name"] == self.allograph.name
