...
def ingest_session(session_id: str, progress: ProgressCallback | None = None) -> dict[str, Any]:
    ...
    session = ImageUploadSession.objects.select_related("item_part", "owner").get(pk=session_id)
    if session.status != ImageUploadSession.Status.ASSEMBLED:
        raise IngestError(f"Session is '{session.status}', expected 'assembled'.")
    session.status = ImageUploadSession.Status.PROCESSING
    ...
    try:
        ...
        with transaction.atomic():
            ...
            session.status = ImageUploadSession.Status.COMPLETE
            session.error = ""
