...
class ImageUploadSession(models.Model):
    ...
    # Media-relative path the served .jp2 will occupy. Derived from the item
    # part and filename, then collision-checked at session creation; it IS
    # the SIPI IIIF identifier, and the key every other session is checked
    # against, so it is stored rather than recomputed.
