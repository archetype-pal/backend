...
def test_full_chunk_flow_and_finalize(management_client, small_chunks, monkeypatch):
    ...
    create = management_client.post(
        SESSIONS_URL,
        _create_payload(size=len(payload)),
        format="json",
    )
...
def test_abort_deletes_session_and_files(management_client, small_chunks):
    session_id = management_client.post(SESSIONS_URL, _create_payload(), format="json").data["id"]
    session = ImageUploadSession.objects.get(pk=session_id)
    assert services.session_tmp_dir(session).exists()

    response = management_client.delete(f"{SESSIONS_URL}{session_id}/")

    assert response.status_code == 204
    assert not ImageUploadSession.objects.filter(pk=session_id).exists()
...
def test_abort_refused_once_assembled_so_a_queued_ingest_keeps_its_input():
    ...
    session = ImageUploadSessionFactory(status=ImageUploadSession.Status.ASSEMBLED)
    ...
    assert ImageUploadSession.objects.filter(pk=session.pk).exists()
...
def test_abort_clears_a_finished_session():
    ...
    session = ImageUploadSessionFactory(status=ImageUploadSession.Status.FAILED)
    client = APIClient()
    client.force_authenticate(user=session.owner)
