...
class TestCreateSession:
    def test_happy_path_computes_destination_and_defaults(self, small_chunks):
        ...
        assert session.status == ImageUploadSession.Status.PENDING
    ...
    def test_other_owners_session_that_never_transferred_is_superseded(self, small_chunks):
        ...
        assert not ImageUploadSession.objects.filter(pk=theirs.pk).exists()
    ...
    def test_writable_destination_dir_passes_even_if_media_root_is_not_writable(self, settings, tmp_path):
        ...
        own directory is already writable (the dev/prod reality where
        media/ belongs to another user but media/uploads/ is opened up)."""
        part = ItemPartFactory()
        media = tmp_path / "media"
        (media / "uploads" / f"item-part-{part.pk}").mkdir(parents=True)
        media.chmod(0o555)
        settings.MEDIA_ROOT = str(media)
        try:
            session = _create_session(item_part=part)
            assert session.destination_path == f"uploads/item-part-{part.pk}/f12r.jp2"
        finally:
            media.chmod(0o755)
...
class TestResumeAfterReload:
    ...
    def test_same_owner_different_size_supersedes_the_stale_attempt(self, small_chunks):
        ...
        assert not ImageUploadSession.objects.filter(pk=first.pk).exists()
...
class TestChunks:
    ...
    def test_receive_is_idempotent_and_tracks_missing(self, small_chunks):
        ...
        assert session.status == ImageUploadSession.Status.UPLOADING
    ...
    def test_chunk_rejected_once_session_left_uploading(self, small_chunks):
        ...
        ImageUploadSession.objects.filter(pk=session.pk).update(status=ImageUploadSession.Status.ASSEMBLED)
        ...
        assert ImageUploadSession.objects.get(pk=session.pk).status == ImageUploadSession.Status.ASSEMBLED
    ...
    def test_409s_instead_of_500ing_when_a_cancel_deletes_the_row(self, small_chunks):
        ...
        ImageUploadSession.objects.filter(pk=session.pk).delete()
...
class TestFinalize:
    ...
    def test_broker_failure_frees_the_destination_instead_of_stranding_it(self, small_chunks, monkeypatch):
        ...
        payload = b"abcdefgh1234"
        owner, part = SuperuserFactory(), ItemPartFactory()
        session = _create_session(owner=owner, item_part=part)
        session = self._upload_all(session, payload)
        monkeypatch.setattr(
        ...
        again = _create_session(owner=owner, item_part=part)
...
class TestCleanup:
    def test_removes_stale_non_complete_sessions(self):
        ...
        done = ImageUploadSessionFactory(status=ImageUploadSession.Status.COMPLETE)
        services.session_tmp_dir(stale).mkdir(parents=True, exist_ok=True)
        old = timezone.now() - timedelta(days=30)
        ImageUploadSession.objects.filter(pk__in=[stale.pk, done.pk]).update(modified=old)
