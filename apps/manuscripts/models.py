...
def get_default_content_type():
    return ContentType.objects.get_for_model(apps.get_model(settings.ITEM_IMAGE_DEFAULT_MODEL)).id
...
class ItemImage(models.Model):
    content_type = models.ForeignKey(
        ...
        limit_choices_to={"model__in": ("ItemPart", "HistoricalItem")},
    )
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "object_id":
            return ContentTypeModelChoiceField(
                queryset=self.content_type.model_class().objects.all(), widget=forms.Select
            )
    ...
    def create_with_default(cls, object_id, **kwargs):
        ...
        return cls.objects.create(content_type=cls.get_default_content_type(), object_id=object_id, **kwargs)
